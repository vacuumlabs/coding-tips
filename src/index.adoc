...
Development^] Another great talk by Rich. The key message is simple:
...
seemingly make all functions pure, but in reality, it's the kind of purity which costs you a lot,
while not buying you either better testability, nor it ease your resoning about the code.

=== Prefer simple data structures

In Java-ish languages one does often define structures like this:
...
```
const me:User = {first_name: "Tomas", last_name: "Kulich"}

function full_name_for_person(person) {
...
you deep updates, there's out-of-the-box (de)serialization with JSON.something, there's an
...
The learning is: study libraries such as lodash. Anytime you're like "hey, I'm forced to
implement something that feels like a general object-manipulation stuff", pause and reflect on your design.
...
manner (time complexity `O(lg(structure_size))`), however, unless the structures are huge, you can
just stick to good-old mutable objects and just not mutating them by convention.
...
We're solving problems. Focus on "what's the problem we're trying to solve here" rather than "what's
the most idiomatic way to achive this in framework X".
...
Always be aware to how much resources (time, memory) you can waste, so you can waste them visely.
With time, be aware of what's the most time-consuming thing that's happening and ignore everything
else, prefering readability to unimportant time-saving. If a reviewer asks you to optimize O(n) work
in O(n**2) algorithm, push back.

Be very humble when predicting what new features bussiness will need. Even if you're right that X
will be needed in 2 months, the prefered way  to achieve X is likely to change by that time. And
...
differently: they quickly produce 100 lines of bad code, then using print and debugger to get the
code in shape. Not only this feels worse, in the end, it's also much more time consuming.
...
after fixing small and easy bugs such as typos), you've won. Soon you develop a healthy ammount of
paranoia, which'll make you question your code frequently. You'll produce a code 3x slower but also
10x faster.

=== Test are good, if they add value

How to write tests is almost a religious thingy, with several though schools. Stay away from the
...
- if test passes, it doesn't mean much; there are many edge cases (e.g. very long user name), that are
  not covered,
- if test fails, you'd probably find out anyway really,
- it doesn't serve as a documentation to something worth documenting.

=== Code reviews are awesome

They are the best tool we have to produce a bug-free, readable and maintainable code. Often, more
than one review is necessary for a code to be merged.
...
The code review process is not so easy to get right.

- Reviewer comments to the PR.
- Author reacts :thumbsup:, if he accepts the proposed change. If it's something more complex, the
  discussion can take place. Avoid writing a lot it's inefficient and frustrating. Have a quick call
  instead.
- Reviewer checks that she likes the outcome by clicking "mark as resolved".
- If reviewer has only cosmettic comments, he can approve the PR indicating he doesn't care that
...
The changes are pushed either as a new "fixing comments" commit, or the history is editted.
Adapt the customs of your project.
...
Unhealthy hype about microservices gave birth to systems with too much microservices and a lot of
...
and you discover using better names such as: `user`, `account`, `ammount` or `transfer()` is good
for your sanity.
Then you code a project with 100k lines, discovering you need even better names:
`account_sender_or_system_account`,
`ammount_USD_before_VAT`, `try_transfer_with_SEPA()`. Often people use
short and imprecise names that kick ass badly, once the project grows.
...
100 characters limit, which looks ugly and not that readable. This is usually less severe
problem as one may think.
...
John: Ah, I see your point. It's optional just for the tests. Real code always puts the value there.
...
Imagine a service that allows you to create new tansactions as such:

```
external_service.add_new_transaction(from: "john", to: "jane", usd_ammount: 10)
```
...
```
try {
  external_service.add_new_transaction({from_account: "john", to_account: "jane", usd_ammount: 10})
} catch (err) {
  // what now?
}
```
...
```
function try_send_transaction_with_repeat({from_account, to_account, usd_ammount, try_this_many_times}) {
  // default value
  ...
  try {
    ...
    if (try_this_many_times > 0) {
      wait(10000)
      try_send_transaction({from_account, to_account, usd_ammount, try_this_many_times - 1})
    } else {
...
your server is faulty)? You'd end up with 10 transaction being sent: uh-oh.
...
```
function try_send_transaction_with_repeat({from_account, to_account, usd_ammount, try_this_many_times, idempotency_key}) {
  // default values
  try_this_many_times = try_this_many_times || 10
  ...
  try {
    ...
    if (try_this_many_times > 0) {
      wait(10000)
      try_send_transaction({from_account, to_account, usd_ammount, try_this_many_times - 1, idempotency_key})
    } else {
      // pun intended
      throw Error('sa mi neda poslat')
    }
